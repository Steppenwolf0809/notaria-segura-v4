  ...
  try {
    ...
    if (cached) {
          ...
          [documents, totalCount] = await Promise.all([
            prisma.document.findMany({
              ...
              include: {
                ...
                invoices: { select: { totalAmount: true, paidAmount: true, status: true, issueDate: true, invoiceNumber: true, invoiceNumberRaw: true } }
              }
            }),
      ...
      documents = documents.map(doc => {
        if (!Array.isArray(doc.invoices) && typeof doc.paymentStatus === 'string') {
          return doc;
        }
