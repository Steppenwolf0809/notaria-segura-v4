...
async function listarTodosDocumentos(req, res) {
  try {
    ...
    if (estado) {
      const norm = String(estado).toUpperCase().trim().split(':')[0];
      const mapped = (norm === 'NOTA_CREDITO' || norm === 'NOTA-CREDITO') ? 'ANULADO_NOTA_CREDITO' : norm;
    ...
    const excludeStatusRaw = excludeEstado ? String(excludeEstado).toUpperCase().trim() : '';
    const excludeStatusSanitized = excludeStatusRaw.split(':')[0];
    const excludeIsValid = VALID_STATUSES.includes(excludeStatusSanitized);
    if (!estado && excludeIsValid) {
      // Solo aplicar exclusión si no se está usando un estado específico
      where.status = { not: excludeStatusSanitized };
    }
    ...
    const cacheKey = cache.key({
      ...
      excludeEstado: excludeIsValid ? excludeStatusSanitized : null,
    ...
    if (searchTerm) {
      ...
      if (supportsUnaccent) {
        try {
          ...
          const filterClauses = [];
          if (estado) filterClauses.push(Prisma.sql`d."status"::text = ${String(estado).toUpperCase().trim().split(':')[0]}`);
          if (!estado && estadosIn.length > 0) filterClauses.push(Prisma.sql`d."status"::text IN (${Prisma.join(estadosIn)})`);
          if (!estado && excludeIsValid) filterClauses.push(Prisma.sql`d."status"::text <> ${excludeStatusSanitized}`);
          ...
          console.warn('Fallo búsqueda con unaccent; usando fallback Prisma:', rawSearchError.message);
          // Fallback con filtros Prisma (case-insensitive)
          where.OR = [
        ...
        // Si no hay unaccent, usar filtros Prisma case-insensitive
        where.OR = [
          { clientName: { contains: searchTerm, mode: 'insensitive' } },
          { clientPhone: { contains: searchTerm, mode: 'insensitive' } },
          { clientEmail: { contains: searchTerm, mode: 'insensitive' } },
