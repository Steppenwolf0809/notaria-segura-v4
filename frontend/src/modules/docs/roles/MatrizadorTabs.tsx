...
import { fetchTrabajoMatrizador, fetchListoMatrizador, fetchEntregadoMatrizador, fetchAnuladoMatrizador, PAGE_SIZE } from '../services/docsQuery';
    ...
    const load = async () => {
      ...
      try {
        if (activeTab === 'trabajo') {
          ...
          const res = await fetchAnuladoMatrizador({ page: pageAnulados, limit: rowsPerPage, search, fechaDesde, fechaHasta, sortOrder });
          const filtered = (res.documents || [])
            .filter((d: any) => ['ANULADO_NOTA_CREDITO','anulado'].includes((d.status || '').toString()))
            .filter((d: any) => matchesSearch(d, search || ''));
          const start = (pageAnulados - 1) * rowsPerPage;
          setDocs(filtered.slice(start, start + rowsPerPage));
